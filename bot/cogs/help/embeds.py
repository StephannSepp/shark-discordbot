...
fortune_embed.add_field(
    ...
    (
        "顯示個人抽籤統計。可接受`group_by`參數指定[運勢|幸運天使]統計的類別\n\n"
        "__**範例**__\n"
        "顯示抽籤各運勢的出現次數、機率```/fortune statistics group_by 運勢```\n"
        "__**範例**__\n"
        "顯示前 9 出現次數最多的幸運天使、機率```/fortune statistics group_by 幸運天使```\n"
    ),
    inline=False,
)
...
vchive_embed.add_field(
    ...
    (
        ...
        "__**範例**__\n"
        "取得特定影片```/vchive archive vid CVG5W9DnJZ8```\n"
        "__**範例**__\n"
        "取得特定頻道```/vchive archive channel AZKi Channel```\n"
    ),
    inline=False,
)
