...
A simple listener to detect the amount of members has changed or not.
...
class Statistics(commands.Cog):
    ...
    async def update_subscription_count(self, before, after):
        if before.member_count != after.member_count:
            member_stats_channel = utils.get(after.voice_channels, id=804256287184388116)
            await member_stats_channel.edit(name=f"Shrimp - {after.member_count:,}")

        if before.premium_subscription_count != after.premium_subscription_count:
            booster_stats_channel = utils.get(after.voice_channels, id=804264456150188043)
            await booster_stats_channel.edit(name=f"Boosters - {len(after.premium_subscribers):,}")
