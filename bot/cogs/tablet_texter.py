...
class TabletTexter(commands.Cog):
    ...
    async def tablet_text(self, inter: CmdInter, text: str):
        ...
        while font.getsize(text)[0] < text_image.size[0] and font.getsize(text)[1] < text_image.size[1]:
            fontsize += 1
            font = ImageFont.truetype("static/NotoSansTC-Thin.woff", fontsize)

        # Draw text
        w, h = text_draw.textsize(text, font=font)
        text_draw.text(xy=((W-w)/2, (H-h)/2), text=text, fill=(0, 0, 0, 255), font=font, align="center")
        # coefs = find_coefs(
        #     [(0, 0), (W, 0), (W, H), (0, H)],
